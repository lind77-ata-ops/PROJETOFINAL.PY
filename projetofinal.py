...
label_endereco  = tk.Label(janela, text='ENDEREÇO: ', font=('arial', 15))
...
btn_salvar.grid(row=0,column=1, padx=2, pady=30) 
btn_editar = tk.Button(janela, text = 'Editar', font= ('arial', 15), command=editar)
btn_editar.grid(row=2,column=5, padx=2, pady=30) 
btn_Deletar = tk.Button(janela, text = 'Deletar', font= ('arial', 15), command=delete_usuario)
btn_Deletar.grid(row=3,column=5, padx=2, pady=30) 
columns = ('NOME', 'EMAIL', 'TELEFONE', 'ENDEREÇO')
tree = ttk.Treeview(janela, columns=columns, show='headings')
tree.grid(row=10, column=0, columnspan=5,padx=10, pady=10)
for col in columns:
